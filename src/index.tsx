...
function getTimeClockView() {
  ...
  return fetch(`${DASHBOARD_URL}/TimeClock/view`, { method: "GET" }).then(
    async (res) => {
      ...
      const pointages = Array.prototype.slice
        .call(html.getElementsByClassName("appClientPointage"))
        .map((pointage) => {
          return {
            href: pointage.getAttribute("href"),
            label: pointage.innerText,
            classname: pointage.classList[1],
          };
        });
      ...
      tempReloadA.addEventListener("click", () => {
      ...
      if (currentPointage.innerText.match(/(?:depuis)/)) {
        ...
        Array.prototype.slice
          .call(html.getElementsByClassName("stopPointage"))
          .forEach((pointage) => {
            const stopPointage = {
              href: pointage.getAttribute("href"),
              label: pointage.innerText,
...
function getTotalTime() {
  ...
  iframe.onload = () => {
    ...
    mutationObserver.observe(
      //@ts-ignore
      iframeDocument.getElementById("WeekPointageRoot"),
      {
        childList: true,
        subtree: true,
      }
